...
typedef struct {//游戏状态
    ...
    // 商店系统相关
    int roomCounter; // 普通房间计数器
    int isStoreRoom; // 当前是否为商店房间 (1=是, 0=否)
    int storeProp1; // 商店道具1类型 (31-37)
    int storeProp2; // 商店道具2类型 (31-37)
    int storeProp1Bought; // 商店道具1是否已购买 (1=是, 0=否)
    int storeProp2Bought; // 商店道具2是否已购买 (1=是, 0=否)
    int storeSpacePressCount; // 空格键按下计数（用于检测双击）
    int storeSpaceLastPressTime; // 上次空格键按下时间
} GameState;
