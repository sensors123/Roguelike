...
void ItsetGameState()//初始化游戏属性
{
    ...
    // 商店系统初始化
    gameState.roomCounter = 0;
    gameState.isStoreRoom = 0;
    gameState.storeProp1 = 0;
    gameState.storeProp2 = 0;
    gameState.storeProp1Bought = 0;
    gameState.storeProp2Bought = 0;
    gameState.storeSpacePressCount = 0;
    gameState.storeSpaceLastPressTime = 0;
}
...
void ShowData()//显示游戏数据
{
    ...
    outtextxy(Width_i *15/20, Hight_i /12, "COIN");
    outtextxy(Width_i *76/100, Hight_i /9, property);
    ...
    // 商店房间提示
    if (gameState.isStoreRoom && gameState.enemyNum <= 0) {
        settextstyle(40, 0, "方正流行体_GBK");
        settextcolor(YELLOW);
        outtextxy(Width_i / 4, Hight_i * 3 / 4, "双击空格进入下一关");
        ...
        // 道具1价格
        char price1[20];
        sprintf(price1, "%d coin", GetPropPrice(gameState.storeProp1));
        outtextxy(Width_i/9 + 3*RoomLength_i - 50, Hight_i/6 + 3*RoomWidth_i + 50, price1);
        
        // 道具2价格
        char price2[20];
        sprintf(price2, "%d coin", GetPropPrice(gameState.storeProp2));
        outtextxy(Width_i/9 + 9*RoomLength_i - 50, Hight_i/6 + 3*RoomWidth_i + 50, price2);
        
        // 如果道具已购买，显示"已购买"
        if (gameState.storeProp1Bought) {
            settextcolor(RED);
            outtextxy(Width_i/9 + 3*RoomLength_i - 50, Hight_i/6 + 3*RoomWidth_i + 80, "已购买");
        }
        if (gameState.storeProp2Bought) {
            settextcolor(RED);
            outtextxy(Width_i/9 + 9*RoomLength_i - 50, Hight_i/6 + 3*RoomWidth_i + 80, "已购买");
        }
    }
}
...
void UpdateEntity()//更新实体
{
    ...
    while (current !=NULL)
    {
        ...
        else if (current->type >20)//道具
        {
            ...
            {
                // 检查是否是商店房间的道具
                int propType = current->type % 10;
                int propPrice = GetPropPrice(propType);
                
                // 如果是商店房间，需要检查coin是否足够
                if (gameState.isStoreRoom) {
                    // 检查道具是否已经被购买
                    int isStoreProp1 = (current->entity.x >= Width_i/9 + 3*RoomLength_i - RoomLength_i/2 && 
                                        current->entity.x <= Width_i/9 + 3*RoomLength_i + RoomLength_i/2 &&
                                        current->entity.y >= Hight_i/6 + 3*RoomWidth_i - RoomWidth_i/2 &&
                                        current->entity.y <= Hight_i/6 + 3*RoomWidth_i + RoomWidth_i/2);
                    int isStoreProp2 = (current->entity.x >= Width_i/9 + 9*RoomLength_i - RoomLength_i/2 && 
                                        current->entity.x <= Width_i/9 + 9*RoomLength_i + RoomLength_i/2 &&
                                        current->entity.y >= Hight_i/6 + 3*RoomWidth_i - RoomWidth_i/2 &&
                                        current->entity.y <= Hight_i/6 + 3*RoomWidth_i + RoomWidth_i/2);
                    
                    if ((isStoreProp1 && gameState.storeProp1Bought) || 
                        (isStoreProp2 && gameState.storeProp2Bought)) {
                        // 道具已经被购买，不能再次购买
                        continue;
                    }
                    
                    // 检查coin是否足够
                    if (Role.property.coin >= propPrice) {
                        // 扣除coin
                        Role.property.coin -= propPrice;
                        
                        // 标记道具为已购买
                        if (isStoreProp1) {
                            gameState.storeProp1Bought = 1;
                        } else if (isStoreProp2) {
                            gameState.storeProp2Bought = 1;
                        }
...
// 获取道具价格
int GetPropPrice(int propType) {
    switch (propType) {
        case 1: return 10; // 午餐
        case 2: return 15; // 丘比特之箭
        case 3: return 20; // 通灵板
        case 4: return 30; // 超凡升天
        case 5: return 8;  // 史蒂文
        case 6: return 5;  // 悲伤洋葱
        case 7: return 5;  // 长条虫
        default: return 0;
    }
}

void GenerateStoreRoom()//生成商店房间
{
    // 清空房间
    for (int i = 0; i < ROW; i++) {
        for (int j = 0; j < COL; j++) {
    ...
    // 创建可用的道具列表（1-7对应道具类型）
    int availableProps[7] = {1, 2, 3, 4, 5, 6, 7};
    int propCount = 7;
    
    // 如果角色已经获得通灵板（道具3），则从列表中移除
    if (Role.state.ghost) {
        for (int i = 0; i < propCount; i++) {
            if (availableProps[i] == 3) {
                for (int j = i; j < propCount - 1; j++) {
                    availableProps[j] = availableProps[j + 1];
                }
                propCount--;
                break;
            }
    ...
    // 如果角色已经获得超凡升天（道具4），则从列表中移除
    if (Role.state.fly) {
        for (int i = 0; i < propCount; i++) {
            if (availableProps[i] == 4) {
                for (int j = i; j < propCount - 1; j++) {
                    availableProps[j] = availableProps[j + 1];
                }
                propCount--;
    ...
    if (propCount < 2) {
        ...
        if (propCount == 0) {
            ...
            availableProps[1] = (availableProps[0] % 7) + 1;
            propCount = 2;
        }
    }
    
    // 随机选择两个不重复的道具
    int prop1Index = rand() % propCount;
    gameState.storeProp1 = availableProps[prop1Index];
    
    // 移除已选择的道具
    for (int i = prop1Index; i < propCount - 1; i++) {
        availableProps[i] = availableProps[i + 1];
    }
    propCount--;
    
    int prop2Index = rand() % propCount;
    gameState.storeProp2 = availableProps[prop2Index];
    
    // 将道具添加到房间中（位置：左侧和右侧）
    // 道具1放在左侧（第3行，第3列）
    Room.room[3][3] = 30 + gameState.storeProp1; // 30 + 道具类型
    
    // 道具2放在右侧（第3行，第9列）
    Room.room[3][9] = 30 + gameState.storeProp2; // 30 + 道具类型
...
void NextLevel()//进入下一个关卡
{
    ...
    // 检查是否需要生成商店房间（每5个普通房间）
    if (gameState.roomCounter % 5 == 0 && gameState.roomCounter > 0 && !gameState.isStoreRoom) {
        // 生成商店房间
        gameState.isStoreRoom = 1;
        GenerateStoreRoom();
    } else {
        // 生成普通房间
        gameState.isStoreRoom = 0;
