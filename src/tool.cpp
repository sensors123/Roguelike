...
void Key()//游戏中按键系统
{
    ...
    if (gameState.isStoreRoom && gameState.enemyNum <= 0) {
        ...
        if (GetAsyncKeyState(VK_SPACE) & 0x8000) {
            int currentTime = gameState.gameFrame_i;
            // 如果是第一次按下或者距离上次按下时间很短（双击检测）
            if (gameState.storeSpacePressCount == 0 || 
                (currentTime - gameState.storeSpaceLastPressTime) < 15) { // 15帧内算双击
                gameState.storeSpacePressCount++;
                gameState.storeSpaceLastPressTime = currentTime;
                
                // 如果连续按了两次空格，进入下一关
                if (gameState.storeSpacePressCount >= 2) {
                    gameState.storeSpacePressCount = 0;
                    gameState.roomFrame_i = 60 * gameState.Roompause; // 立即进入下一关
                }
            }
        } else {
            // 如果空格键释放，重置计数（但保留时间信息用于双击检测）
            // 这里不重置计数，允许在短时间内再次按下
        }
        
        // 如果超过30帧没有再次按下空格，重置计数
        if (gameState.storeSpacePressCount > 0 && 
            (gameState.gameFrame_i - gameState.storeSpaceLastPressTime) > 30) {
            gameState.storeSpacePressCount = 0;
        }
    } else {
        // 不在商店房间，重置计数
